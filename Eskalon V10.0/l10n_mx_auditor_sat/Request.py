...
class RequestDownloadRequest:
    ...
    # depende del SAT
    def soapRequest(self, certificate, keyPEM, token, start_date, end_date,
                    tipo_solicitud = XML_FACTURAS, rfc_data={}):
        xml = self.getSOAPBody(certificate=certificate, keyPEM=keyPEM, start_date=start_date, end_date=end_date,
                               tipo_solicitud=tipo_solicitud, rfc_data=rfc_data)
        print ("### XML >>> ",xml)
    ...
    def getSOAPBody(self, certificate, keyPEM, start_date, end_date,
                    tipo_solicitud = XML_FACTURAS, rfc_data={}):
        
        #rfc = Utils.rfc_from_certificate(certificate)
        rfc = rfc_data['rfc_emisor']
        ...
        data = '<des:SolicitaDescarga xmlns:des="http://DescargaMasivaTerceros.sat.gob.mx">' \
               '<des:solicitud RfcEmisor="{rfc}" RfcReceptor="{rfc}" RfcSolicitante="{rfc}" FechaInicial="{fecha_inicial}" ' \
        ...
        # print ("### DATA TO SIGN SOAP >>> ",dataToSign)
