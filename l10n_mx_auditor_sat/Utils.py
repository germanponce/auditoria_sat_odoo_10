...
def xml_etree(xml):
    ...
    '''
    xslt_doc = etree.parse(BytesIO(xslt_remove_namespaces.encode('utf8')))
...
def b64_signature_pkey(dataToSign, pkeyPEM):
    ...
    # #### GERMAN PRUEBAS ###
    # transformedDocument = str(dataToSign).encode('UTF-8')
    # crypter = RSA.load_key_string(pkeyPEM)
    # algrthm = MessageDigest('sha1') # Contabilidad 1.3
    # algrthm.update(transformedDocument)
    # rawStamp = crypter.sign(algrthm.digest(), 'sha1') # Contabilidad 1.3
    # rawStamp_b64 = b64enc(rawStamp)
    # print ("***************************************************")
    # print ("####### rawStamp_b64 >>> ",rawStamp_b64)
    # print ("***************************************************")
    # print ("### crypter >>> ",crypter)
    # print ("#### rawStamp >>> ",rawStamp)
...
def b64_sha1_digest(data):
    digest_value = base64.b64encode(hashlib.sha1(data.encode()).digest()).decode('utf8')
    return digest_value


def issuer_data_string(certificate):
...
def get_subject_utf8components(certificate):
    certificate = crypto.load_certificate(crypto.FILETYPE_ASN1, certificate)
...
def rfc_from_certificate(certificate):
...
def b64_certificate(certificate):
    return base64.b64encode(certificate).decode('utf8')


# def pkey_to_pem(public_key_path, password):
...
# def pkey_buffer_to_pem(private_key, password):
#     key = chilkat2.PrivateKey()
