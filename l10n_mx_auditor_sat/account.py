...
class CFDIAccountDashboardManager(models.Model):
    ...
    def check_download_pending(self, file_globals, id_solicitud):
        ...
        if codigo_estado_solicitud != '5000':
            ...
            ### Paquete en Proceso de Solicitud ###
            elif estado_solicitud == '2':
                numero_cfdis = data_solicitud_check['numero_cfdis']
                number_of_documents = numero_cfdis ### Total Descargado en el SAT
                _logger.info('\n################ Solicitud en Proceso ( %s ).....' % self.name)
                if data_solicitud_check['paquetes']:
                    paquetes = list(data_solicitud_check['paquetes'])
                else:
                    paquetes = []
                _logger.info("\n################ PAQUETES %s " % paquetes)
                ...
                download_pending = True
                no_data = False
                status_solicitud = 'Estado de Solicitud en Proceso.'
    ...
    def _run_download_pending_package_from_sat(self):
        ...
        for consulta in records_pending:
            ...
            write_date = datetime.strptime(str(write_date), '%Y-%m-%d %H:%M:%S')
            date_act = fields.Datetime.now()
            date_act = datetime.strptime(str(date_act), '%Y-%m-%d %H:%M:%S')
...
class AccountCFDIMultiDownload(models.TransientModel):
    ...
    def vat_b64_str_to_physical_file(self, b64_str, file_extension, prefix='data'):
        _logger.info("\n####################### b64_str_to_physical_file >>>>>>>>>>> ")
        _logger.info("\n####################### file_extension %s " % file_extension)
        _logger.info("\n####################### prefix %s " % prefix)
        certificate_lib = self.env['facturae.certificate.library']
    ...
    def _get_file_globals_download(self):
        ...
        if rfc_solicitante[0:2] == 'MX':
            rfc_solicitante = rfc_solicitante[2:]
        rfc_to_txt, rfc_to_txt_path = self.vat_b64_str_to_physical_file(rfc_solicitante,'txt','rfc')
    ...
    @api.multi
    def get_token_cfdi_client(self, cer_der, key_der, fiel_pas):        
        vals_consumo = {
                             'file_cer_path': cer_der,
                             'file_key_path': key_der,
                             'file_pass': fiel_pas,

            }

        # ### TOKEN ####
        cliente = xmlrpclib.ServerProxy('http://localhost:8066')
        try:
            result_tk_vls = cliente.get_token_as_var(vals_consumo)
        except Exception as e:
            raise UserError("Ocurrio un error al generar la consulta.\nError: %s" % e)
        token_ws  = result_tk_vls['token']
    ...
    def get_download_request(self, file_globals, download_type='emitidos'):
        ...
        for rec in self:
            ...
            rfc_txt_path = file_globals['temporary_path_files']['rfc_txt_path']
            ...
            vals_consumo = {
                             ...
                             'rfc_txt_path': rfc_txt_path,
                             'file_pass': FIEL_PAS,
                             'rfc_solicitante': str(rfc_solicitante),
                             'fecha_inicial': fecha_inicial,
                             'fecha_final': fecha_final,
                             'rfc_emisor': str(rfc_emisor),
                             'rfc_receptor': str(rfc_receptor),
                             'token': file_globals['token'],
    ...
    def get_download_check_request(self, file_globals, id_solicitud):
        ...
        try:
            result = cliente.verificar_descarga(vals_consumo, rfc_solicitante, id_solicitud)
        except Exception as e:
            raise UserError("Ocurrio un error al generar la consulta indicada.\nError: %s" % e)
    ...
    def get_download_final_data_request(self, file_globals, id_paquete):
        ...
        try:
            result = cliente.descargar_paquete(vals_consumo, rfc_solicitante, id_paquete)
        except Exception as e:
            raise UserError("Ocurrio un error al generar la consulta.\nError: %s" % e)
    ...
    def execute_download(self):
        for rec in self:
            ...
            if cod_estatus != '5000':
                ...
                id_solicitud = data_solicitud.get('id_solicitud','')
            else:
                ...
                if codigo_estado_solicitud != '5000':
                    ...
                    ### Paquete en Proceso de Solicitud ###
                        ...
                        numero_cfdis = data_solicitud_check['numero_cfdis']
                        number_of_documents = numero_cfdis ### Total Descargado en el SAT
                        _logger.info('\n################ Solicitud en Proceso ( %s ).....' % rec.name)
                        if data_solicitud_check['paquetes']:
                            paquetes = list(data_solicitud_check['paquetes'])
                        else:
                            paquetes  = []
