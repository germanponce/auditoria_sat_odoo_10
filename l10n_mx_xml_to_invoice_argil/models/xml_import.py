...
class XmlImportWizard(models.TransientModel):
    ...
    def validate_bills(self):
        ...
        if invoice_ids:
            ...
            raise UserError("No se encontro información para la creación o busqueda de facturas.")
    ...
    def action_view_invoices(self, invoice_ids):
        ...
        action = self.env.ref(action_ref).sudo()
    ...
    def get_partner_or_create(self, partner):
        ...
        search_domain = [
         (
          'vat', '=', partner['rfc'])]
        if self.invoice_type == 'out_invoice' or self.invoice_type == 'out_refund':
            search_domain.append(('customer_rank', '=', True))
        else:
            search_domain.append(('supplier_rank', '=', True))
    ...
    def get_uom(self, sat_code):
        ...
        return ProductUom.search([('sat_uom_id.code', '=', sat_code)], limit=1)
    ...
    def get_product_or_create(self, product):
        ...
        if not p:
            if self.create_product:
                ...
                if sat_code:
                    product_vals['sat_product_id'] = sat_code.id
